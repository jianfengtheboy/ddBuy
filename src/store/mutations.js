...
export default {
  // 1.添加商品
  [types.ADD_GOODS](state, { goodsID, goodsName, smallImage, goodsPrice }) {
  ...
  // 3.减少商品
  [types.REDUCE_GOODS](state, { goodsID }) {
    // 3.1 取出state中的商品数据
    let shopCart = state.shopCart
    // 3.2 通过商品ID来找到这个商品
  ...
  // 4.单个商品选中
  [types.SINGLE_SELECT_GOODS](state, { goodsID }) {
    // 4.1 取出state中的商品数据
    let shopCart = state.shopCart
    // 4.2 根据商品id取到goods
    let goods = shopCart[goodsID]
  ...
  // 10.用户生日
  [types.USER_INFO_BRITHDAY](state, { brithday }) {
    // 10.1 取出state中的用户信息
    let userInfo = state.userInfo
